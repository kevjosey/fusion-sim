...
gen_data <- function(n, sig2 = 5, scenario = c("base", "ps-mis", "out-mis", "ps-exchange",
  ...
  if (scenario == "base"){
    ...
    mu_1 <- mu_0 + U %*% alpha
    PATE <- mean(U[s == 0,]%*%alpha)
  } else if (scenario == "out-exchange"){
    f_X <- c(1/(1 + exp( -( U %*% gamma) ) ))
    s <- rbinom(n, 1, f_X) # sample
    e_X <- c(1/(1 + exp( -( U %*% lambda) ) ))
    z <- rbinom(n, 1, e_X) # treatment
    mu_0 <- s*(X %*% beta0) + (1 - s)*(X %*% beta1)
    mu_1 <- mu_0 + X %*% alpha
    PATE <- mean(X[s == 0,]%*%alpha)
...
simfit <- function(idx = 1, simDat, ...) {
  ...
  est_t <- try( calib(S = S, Y = Y, Z = Z, X = X, fusion = FALSE), silent = TRUE )
  cal_t <- ifelse(!inherits(est_t, "try-error") & est_t$estimate < 14 & est_t$estimate > -10, est_t$estimate, NA)
  var_t <- ifelse(!inherits(est_t, "try-error") & est_t$estimate < 14 & est_t$estimate > -10, est_t$variance, NA)
  
  # Calibration - Fusion
  
  est_f <- try( calib(S = S, Y = Y, Z = Z, X = X, fusion = TRUE), silent = TRUE )
  cal_f <- ifelse(!inherits(est_f, "try-error") & est_f$estimate < 14 & est_f$estimate > -10, est_f$estimate, NA)
  ...
  tmle_f <- try( tmle(S = S, Y = Y, Z = Z, X = X, fusion = TRUE), silent = TRUE )
  tmle_est_f <- ifelse(!inherits(tmle_f, "try-error") & tmle_f$estimate < 14 & tmle_f$estimate > -10, tmle_f$estimate, NA)
  tmle_var_f <- ifelse(!inherits(tmle_f, "try-error") & tmle_f$estimate < 14 & tmle_f$estimate > -10, tmle_f$variance, NA)
  
  # Augmented - Transport
  
  aug_t <- try( aug(S = S, Y = Y, Z = Z, X = X, fusion = FALSE), silent = TRUE )
  aug_est_t <- ifelse(!inherits(aug_t, "try-error") & aug_t$estimate < 14 & aug_t$estimate > -10, aug_t$estimate, NA)
  aug_var_t <- ifelse(!inherits(aug_t, "try-error") & aug_t$estimate < 14 & aug_t$estimate > -10, aug_t$variance, NA)
  ...
  tau <- c(tmle_est_t,aug_est_t,cal_t,tmle_est_f,aug_est_f,cal_f)
  se <- c(sqrt(tmle_var_t),sqrt(aug_var_t),sqrt(var_t),sqrt(tmle_var_f),sqrt(aug_var_f),sqrt(var_f))
